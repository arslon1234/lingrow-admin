...
function processEngineHoursAndStatus(events) {
    ...
    processedEvents = processedEvents.map(event => {
        event.timestamp = dayjs(event.dateTime).valueOf();
        return event;
    });

    // Filter out ignored statuses using a Set for O(1) lookups.
    processedEvents = processedEvents.filter(event => !IgeSet.has(event.eventCodeDescription));

    // Extract engine status changes
    const engineStatusChanges = processedEvents.filter(event =>
      event.eventCodeDescription === NORMAL_ENGINE_UP ||
      event.eventCodeDescription === NORMAL_ENGINE_DOWN
    );

    // Compute maximum engine hours among all events
    let maxEngineHours = -Infinity;
    for (let i = 0, len = events.length; i < len; i++) {
        if (events[i] && events[i].totalEngineHours > maxEngineHours) {
            maxEngineHours = events[i].totalEngineHours;
        }
    }
...
function checkEngineHoursMismatch(event, actualDiff, expectedDiff, eventsArr) {
