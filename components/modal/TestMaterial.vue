    ...
    <UModal v-model="internalValue" :ui="{ base: 'sm:!max-w-[500px]' }">
        <div class="w-full p-6 space-y-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold">{{ isEditMode ? `Update ${materialType === 'MOCK' ? 'Test' :
                    'Unit'}` : `Create New ${materialType === 'MOCK' ? 'Test' : 'Unit'}` }}</h2>
            ...
            <form @submit.prevent="handleSubmit" class="space-y-4" id="form">
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                        {{ `${materialType === 'MOCK' ? 'Test' : 'Unit'}` }} number <span class="text-red-500">*</span>
                    </label>
