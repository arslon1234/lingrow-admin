	...
	<main class="h-screen flex flex-col">
		...
		<div class="bg-white border-b border-gray-200 px-6 py-4">
			<div class="flex items-center justify-between">
				<h1 class="title">Listening Materials</h1>
				<UButton @click="openCreateModal" icon="i-heroicons-plus" size="md" color="primary"> Create Book </UButton>
			</div>
		</div>
		...
		<div class="flex flex-1 overflow-hidden">
			...
			<aside class="w-80 bg-white border-r border-gray-200 flex flex-col">
				...
				<div class="p-4 space-y-3 border-b border-gray-200">
					<UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search books..." size="md" />

					<!-- TYPE FILTER -->
					<USelectMenu v-model="selectedBookType" :options="bookTypeOptions" value-attribute="value" option-attribute="label" placeholder="All Types" size="md">
						<template #label>
					...
					<!-- STATUS FILTER -->
					<USelectMenu v-model="selectedStatus" :options="statusOptions" value-attribute="value" option-attribute="label" placeholder="All Status" size="md">
						<template #label>
				...
				<div class="flex-1 overflow-y-auto">
					...
					<div v-else class="p-2">
						<button
							...
							class="w-full text-left px-4 py-3 rounded-lg mb-2 transition-all border-2 bg-white hover:bg-gray-50"
							:class="{
								'border-blue-500 bg-blue-50': selectedBookId == book.id,
						...
						>
							<div class="flex items-start justify-between gap-2">
								...
								<UIcon
									...
									:class="{
										'text-blue-500': selectedBookId == book.id,
										'text-gray-400': selectedBookId != book.id
									}"
								/>
							</div>
						</button>
			...
			<div class="flex-1 overflow-y-auto bg-gray-50">
				<div v-if="!selectedBookId || !selectedBook?.title" class="h-full flex items-center justify-center">
						...
						<div class="flex items-start justify-between mb-4">
							<div class="flex-1">
								...
								<div class="flex items-center gap-3 text-sm">
									...
									<span class="px-3 py-1 rounded-lg font-medium" :class="getStatusClass(selectedBook.status)">
										{{ selectedBook.status }}
									</span>
									<span v-if="selectedBook.publisher" class="text-gray-600"> Published by {{ selectedBook.publisher }} </span>
								</div>
							</div>

							<!-- ACTION BUTTONS -->
							<div class="flex items-center gap-2 shrink-0">
								<UButton icon="i-heroicons-pencil-square" size="md" color="gray" variant="soft" @click="handleEdit(selectedBook)"> Edit </UButton>
								<UButton icon="i-heroicons-trash" size="md" color="red" variant="soft" @click="handleDelete(selectedBook.id)"> Delete </UButton>
							</div>
						</div>
					</div>
								...
								<div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-colors cursor-pointer">
									<div class="flex items-center gap-3 mb-2">
										<div class="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center">
											<UIcon name="i-heroicons-chat-bubble-left-right" class="w-5 h-5 text-green-600" />
										</div>
										<div>
