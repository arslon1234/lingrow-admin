<template>
	<main>
		<UModal v-model="eldConnectionModal" :ui="{ base: 'sm:!max-w-[420px]' }">
			<div class="w-full p-6 space-y-6">
				<div class="flex items-center justify-between">
					<h2 class="title">
						{{ editEldConnectionModal ? 'Edit Eld Connection' : 'Add Eld Connection' }}
					</h2>
					<UButton color="gray" :ui="{ rounded: 'rounded-full' }" variant="ghost" icon="i-heroicons-x-mark-20-solid"
						class="-my-1" @click="closeEldConnectionModal" />
				</div>
				<UDivider />
				<UForm :validate="validateEldConnection" :state="eldConnectionForm" @submit="() => console.log('123')">
					<div class="w-full space-y-6">
						<UFormGroup v-slot="{ error }" label="Name" name="name">
							<UInput size="lg" searchable v-model="eldConnectionForm.name" placeholder="Name"
					...
					<div class="space-y-6 mt-6">
						...
						<div class="flex items-center justify-end gap-x-3">
							<UButton @click="closeEldConnectionModal" class="w-28 justify-center dark:bg-dark-button-0 dark:hover:bg-dark-button-0/[0.5]" size="xl"
								label="Cancel" variant="solid"
								:ui="{ rounded: 'rounded-lg', variant: { solid: 'bg-grey-2 hover:bg-grey-1' } }" />
							<UButton :disabled="isAddOrUpdateEldConnectionDisabled" :loading="loading" @click="updateOrAddEldConnection"
								type="submit" class="w-28 justify-center" size="xl" :label="editEldConnectionModal ? 'Edit' : 'Create'"
		...
		<UModal v-model="deleteModal" :ui="{ base: 'sm:!max-w-[450px]' }">
			<div class="w-full p-6 space-y-6">
				<div class="flex items-center justify-between">
					<h2 class="title">Eld Connections</h2>
					<UButton color="gray" :ui="{ rounded: 'rounded-full' }" variant="ghost" icon="i-heroicons-x-mark-20-solid"
				...
				<div class="flex items-center justify-end gap-x-3">
					...
					<UButton :disabled="loading" :loading="loading" @click="deleteEldConnection" class="w-28 justify-center"
						size="xl" label="Delete" variant="solid" :ui="{ rounded: 'rounded-lg' }" />
				</div>
			</div>
		</UModal>
		<section class="card flex justify-between items-center">
			<h1 class="title text-lg">Eld Connections</h1>
			<div class="flex items-center gap-x-3">
				<UButton @click="openAddEldConnectionModal" size="sm" variant="solid" label="Add" />
			</div>
		</section>
		<section class="table_config">
			<UTable :columns="columns" :rows="rows">
				<template #action-data="{ row }">
					<div class="flex gap-x-2.5 items-center">
						<UButton @click="openEditEldConnectionModal(row)" size="sm" variant="soft"
							:ui="{ variant: { soft: 'bg-purple/[.2] hover:bg-purple/[.4] text-purple' } }">
							<template #leading>
								<EditIcon class="text-purple" />
							</template>
						</UButton>
						<UButton @click="() => openDeleteEldConnectionModal(row)" size="sm" variant="soft"
			...
			<UPagination v-if="eldConnectionsTotal > pageSize" class="mt-5"
				...
				:page-count="pageSize" :total="eldConnectionsTotal" v-model="currentPage" show-first show-last :ui="{
					wrapper: 'mx-auto'
				}" />
		</section>
	</main>
...
import { useConfigurationsEldConnections } from '~/composables/pages/configurations/eld-connections';
...
const {
	...
	columns,
	rows,
	deleteEldConnection, } = await useConfigurationsEldConnections();
</script>
